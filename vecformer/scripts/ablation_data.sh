...
# Each run: same model from scratch, 60k steps, best checkpoint on 3 held-out US
# projects, then one evaluation on the US test split. Summary: tools/ablation_report.py.
#
#   cd vecformer && bash scripts/ablation_data.sh                 # A, B, C in turn
...
for r in $RUNS; do
    ...
    run=$OUT/$r
    echo "== run $r: $data -> $run"
    python ../cloud/dryrun_joint.py --data "$data" --model configs/model/ablation_arch43.yaml
    torchrun --nproc_per_node="$NPROC_PER_NODE" --master_port=$((11460 + RANDOM % 100)) launch.py \
        --launch_mode train --config_path "$TRAIN_CFG" \
        --model_args_path configs/model/ablation_arch43.yaml --data_args_path "$data" \
        --run_name "ablation_$r" --save_total_limit 2 --output_dir "$run" \
        2>&1 | tee "$OUT/$r.train.log"
    best=$(python - "$run" <<'PY'
...
PY
    ...
    torchrun --nproc_per_node="$NPROC_PER_NODE" --master_port=$((11560 + RANDOM % 100)) launch.py \
        --launch_mode test --resume_from_checkpoint "$best" --config_path "$TRAIN_CFG" \
        --model_args_path configs/model/ablation_arch43.yaml --data_args_path "$data" \
        --run_name "ablation_${r}_test" --output_dir "$run/test" \
        2>&1 | tee "$OUT/$r.test.log"
done
