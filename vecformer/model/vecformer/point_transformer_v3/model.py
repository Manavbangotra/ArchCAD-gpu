"""
Modified from https://github.com/Pointcept/PointTransformerV3
...
class PointTransformerV3(PointModule):
    ...
    def _pooling_feats(self,
            ...
            sampled_cu_seqlens: torch.Tensor,
            primitive_id_map: torch.Tensor
        ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Pooling sampled features to primitive features

        Args:

            `sampled_feats` (`torch.Tensor`, shape is (N1+N2+..., feats_embed_dim)): Features of sampled primitives
                N1, N2, ... are the number of sampled primitives in each batch

            `sampled_cu_seqlens` (`torch.Tensor`, shape is (batch_size + 1,)): Cumulative sequence lengths of sampled primitives
                The first element is 0, and the last element is the total number of sampled primitives in all batches

            `primitive_id_map` (`torch.Tensor`, shape is (N1+N2+...,)): Primitive id map
                Indicates the primitive id of each sampled primitive, e.g.
                ```
                primitive_id_map[0] = 0
                ```
                means the 0-th sampled primitive belongs to the 0-th primitive
        ...
        """
        prim_feats = []
        for batch_idx in range(len(sampled_cu_seqlens) - 1):
            idx_start = sampled_cu_seqlens[batch_idx]
            idx_end = sampled_cu_seqlens[batch_idx + 1]
            sampled_feats_batch = sampled_feats[idx_start:idx_end]
            primitive_id_map_batch = primitive_id_map[idx_start:idx_end]
            pooled_feats = GroupFeatFusion._group_pooling(
                sampled_feats_batch, primitive_id_map_batch)
