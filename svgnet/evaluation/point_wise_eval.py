"""
Source: https://github.com/nicehuster/SymPointV2/blob/master/svgnet/evaluation/point_wise_eval.py

NOTICE: Original repository does not specify a license.
This code is used for research purposes only.
Will be removed or replaced if licensing issues arise.
"""

import numpy as np
from svgnet.data.svg import SVG_CATEGORIES, get_categories
import torch
import torch.distributed as dist

class PointWiseEval(object):
    def __init__(self, num_classes=30, ignore_label=30,gpu_num=1) -> None:
        ...
        self._class_names = [x["name"] for x in get_categories(num_classes)[:-1]]
...
class InstanceEval(object):
    def __init__(self, num_classes=30,
        ...
        # Derive countable ("thing") vs uncountable ("stuff") classes from the
        # taxonomy's isthing flag rather than hardcoding indices. The previous
        # literal lists were written for a 30-class ArchCAD setup and indexed out
        # of bounds on any smaller taxonomy, crashing panoptic evaluation.
        cats = get_categories(num_classes)[:num_classes]
        self.thing_class = [i for i, c in enumerate(cats) if c.get("isthing", 0)]
        self.stuff_class = [i for i, c in enumerate(cats) if not c.get("isthing", 0)]
