"""
Source: https://github.com/nicehuster/SymPointV2/blob/master/svgnet/model/utils.py
...
class TorchDict(dict):
    ...
    def cuda(self, inplace=True, try_fn=True):
        ...
        return traverse_dict(self, lambda x: x.to(get_device()), inplace=inplace, try_fn=try_fn)
...
class TorchList(list):
    ...
    def cuda(self, inplace=True, try_fn=True):
        from ..util.utils import get_device
        return traverse_list(self, lambda x: x.to(get_device()), inplace=inplace, try_fn=try_fn)
