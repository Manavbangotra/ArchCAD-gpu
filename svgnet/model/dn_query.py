"""
Source: https://github.com/nicehuster/SymPointV2/blob/master/svgnet/model/dn_query.py
...
class CDNQueries(object):
    ...
    def calc_indices(self, targets):
        ...
        map_known_indices = torch.cat(map_known_indices).long().to(targets[0]["labels"].device)
    ...
    def query_for_dn(self,queries,targets,label_enc):
        ...
        padding = torch.zeros([bs, pad_size, queries.shape[-1]], device=queries.device)
        ...
        tgt_mask = torch.ones(tgt_size, tgt_size, device=queries.device) < 0
    ...
    def mask_for_dn(self,stage_list,step):
        ...
        padding_mask = torch.ones([bs, pad_size, group_num], device=masks.device).bool()
