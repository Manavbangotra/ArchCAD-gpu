...
Source: https://github.com/nicehuster/SymPointV2/blob/master/modules/pointnet2_utils.py
...
def sample_and_group(stride, nsample, xyz, points, offset, return_idx=False, num_sector=1):
    ...
    if stride > 1:
        ...
        new_offset = torch.tensor(new_offset, dtype=torch.int32, device=xyz.device)
        if num_sector > 1:
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, pos_feat_off1, pos_feat_off2):
        ...
        interpolated_points = torch.zeros(xyz1.shape[0], points2.shape[1],
                                          dtype=points2.dtype, device=points2.device)
        for i in range(3):
